...
# Entry 2: Direct Move
source_df = source_df.withColumn("BALANCE_AMOUNT", col("TRANS_AMT").cast("decimal(20,2)"))

# Entry 3: Convert age to age group
source_df = source_df.withColumn("AGE_GROUP", 
                                 when(col("AGE").between(20,30), "20-30")
                                 .when(col("AGE").between(31,40), "31-40")
                                 # Add more when conditions for other age groups
                                 .otherwise("Unknown"))
