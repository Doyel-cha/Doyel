...
from pyspark.sql.functions import lpad, col, when, lit
...
def source_df(spark_session):
    ...
    data = [
        (1, "1234", 1000.00, 25, 10, 100.00),
        (2, "56789", 500.50, 35, 5, 50.00)
    ]
    # Define schema
    schema = ["TRANS_ID", "ACCOUNT_NO", "TRANS_AMT", "AGE", "QUANTITY", "UNIT_PRICE"]
    # Create DataFrame
...
def test_account_number_padding(source_df):
    ...
    assert result_df.select("ACCOUNT_NIUMBER").collect()[0][0] == "0000000000001234"

# Test for Entry 2: Direct Move
def test_balance_amount_transformation(source_df):
    result_df = source_df.withColumn("BALANCE_AMOUNT", col("TRANS_AMT").cast("decimal(20,2)"))
    assert result_df.select("BALANCE_AMOUNT").collect()[0][0] == 1000.00

# Test for Entry 3: Convert age to age group
def test_age_group_transformation(source_df):
    result_df = source_df.withColumn("AGE_GROUP", 
...
def test_total_price_calculation(source_df):
    ...
    assert result_df.select("TOTAL_PRICE").collect()[0][0] == 1000.00
    assert result_df.select("TOTAL_PRICE").collect()[1][0] == 250.00
```
